...
/**
 ...
 Lifecycle information for current session should be attached to queued hit or as seperate hit (If we have no queued hit) for every lifecyle session. (We get this information from `lifecycle.responseContent` event)
 Referrer information for current install/launch should be attached to queued hit or as seperate hit (If we have no queued hit) (We get this information from `acquisition.responseContent` event)

 Given that Lifecycle, Acqution and MobileServices extensions are optional we rely on timeouts to wait for each of the above events and reorder hits
 ...
 If we receive `lifecycle.responseContent` before timeout, we append lifecycle data to first waiting hit. It is sent as a seperate hit if we don't have any waiting hit
 ...
 If we receive `acquisition.responseContent` before timeout, we append lifecycle data to first waiting hit. It is sent as a seperate hit if we don't have any waiting hit
